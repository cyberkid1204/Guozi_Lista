    ...
    try:
        ...
        if process_type in [0, '0']:
            ...
            if self.source != self.dest:
                ...
                process_mode = 5
        elif process_type in [1, '1']:  # 入库
            """入库流程起点为取满箱或取空箱区，点位不是区域的叶子点位"""
            check_src_in_area = not await is_area(self=self, location_name=src)
            ...
            if not (check_src_in_area and check_dst_in_area):
                ...
                return 504
            process_mode = 2
        elif process_type in [2, '2']:  # 出库
            """出库流程：满箱起点是库区，终点是机械臂；空箱起点为空箱储存区终点是机械臂"""
...
# 工位入库业务
async def into_warehouse_task(self, src, dst, lock, waiting_point=None, need_post=2):
    """
    :param self: ==self
    :param src: 起点
    :param dst: 终点
    ...
    if self.source in location_check_point_mapping:
        ...
        if not waiting_point:
            waiting_point = waiting_area_check_point.get(
                waiting_area_name_list[0])
        ...
        while True:
            ...
            if ((time_spend_in_waiting_end - time_spend_in_waiting_start) > time_spend_in_waiting_limit) \
                ...
                require_result, new_dst = await require_new_dst(self, request_data={"orderID": self.order.order_id,
                                                                                    ...
                                                                                    "operationType": "1"})
                self.logger.info(
                    f'''{str(require_result)}________________{str(new_dst)}''')
...
async def is_area(self, location_name):
    ...
    if not self.father or not self.position:
        ...
        await self.set_gp('position', json.dumps(self.position), 'str')
    try:
        flag1 = await self.get_gp('docks')  # 对应的值为 '1'（字符串）
    except:
        flag1 = None
    try:
        flag2 = await self.get_gp('rowcol')  # 对应的值为 '1'（字符串）
    except:
        flag2 = None
    if flag1 is None:
        await read_docks(self, xmlfile)
...
async def cal_weight(sxmin, sxmax, symin, symax, dxmin, dxmax, dymin, dymax, sx, sy, dx, dy):
    ...
    w1 = 2
    w2 = 1
    point_list['ul'] = (abs(symax - sy) + abs(dxmax - dx)) * \
        w1 + (abs(dxmax - sx) + abs(symax - dy)) * w2
    point_list['uu'] = (abs(symax - sy) + abs(dymin - dy)) * \
        w1 + (abs(symax - dymin) + abs(dx - sx)) * w2
    point_list['ud'] = (abs(symax - sy) + abs(dymax - dy)) * \
        w1 + (abs(symax - dymax) + abs(dx - sx)) * w2
    point_list['ur'] = (abs(symax - sy) + abs(dxmin - dx)) * \
        w1 + (abs(symax - dy) + abs(dxmin - sx)) * w2
    point_list['dl'] = (abs(symin - sy) + abs(dxmax - dx)) * \
        w1 + (abs(symin - dy) + abs(sx - dxmax)) * w2
    point_list['du'] = (abs(symin - sy) + abs(dymin - dy)) * \
        w1 + (abs(symin - dymin) + abs(sx - dx)) * w2
    point_list['dd'] = (abs(symin - sy) + abs(dymax - dy)) * \
        w1 + (abs(symin - dymax) + abs(sx - dx)) * w2
    point_list['dr'] = (abs(symin - sy) + abs(dxmin - dx)) * \
        w1 + (abs(symin - dy) + abs(dxmin - sx)) * w2
    point_list['ll'] = (abs(sxmin - sx) + abs(dxmax - dx)) * \
        w1 + (abs(sxmin - dxmax) + abs(sy - dy)) * w2
    point_list['lu'] = (abs(sxmin - sx) + abs(dymin - dy)) * \
        w1 + (abs(sxmin - dx) + abs(dymin - sy)) * w2
    point_list['ld'] = (abs(sxmin - sx) + abs(dymax - dy)) * \
        w1 + (abs(sxmin - dx) + abs(dymax - sy)) * w2
    point_list['lr'] = (abs(sxmin - sx) + abs(dxmin - dx)) * \
        w1 + (abs(sxmin - dxmin) + abs(sy - dy)) * w2
    point_list['rl'] = (abs(sxmax - sx) + abs(dxmax - dx)) * \
        w1 + (abs(sxmax - dxmax) + abs(sy - dy)) * w2
    point_list['ru'] = (abs(sxmax - sx) + abs(dymin - dy)) * \
        w1 + (abs(sxmax - dx) + abs(dymin - sy)) * w2
    point_list['rd'] = (abs(sxmax - sx) + abs(dymax - dy)) * \
        w1 + (abs(sxmax - dx) + abs(dymax - sy)) * w2
    point_list['rr'] = (abs(sxmax - sx) + abs(dxmin - dx)) * \
        w1 + (abs(sxmax - dxmin) + abs(sy - dy)) * w2
    ...
    if 'storehouse' in is_position[0]['location_name']:
        if way == 1:
            ...
            if to == 'l' or to == 'r':
                ...
                self.logger.debug(
                    to + row_col[0]['gp_name'] + 'ggggggggggggggggggggg')
                if to == 'r':
...
async def route_cal(self, src_name, dst_name):
    ...
    if len(src_position[1]) == 1 and len(dst_position[1]) > 1:
        ...
        return None, None
    print(smin_x, smax_x, smin_y, smax_y, dmin_x,
          dmax_x, dmin_y, dmax_y, sx, sy, dx, dy)
...
async def function_main(self, base_url=None, scan_type=None, location=None,
    ...
    if self.process_type == '2' or self.process_type == '3':
        try:
            ...
            self.logger.error(
                f"+++++++++++++{self.order.order_id} {str(e)}++++++++++++")
            self.logger.error(
                f"+++++++++++++{self.order.order_id} 删除托盘失败++++++++++++")
        ...
        while (1):
            try:
                requests.post(f'{base_url}/upload_QRcode',
                              data=json.dumps(
                                  {"orderID": rfid_result[0], "QR_code": cargo_code, "location": dst}),
                              headers=headers)
                break
            except Exception as e:
                self.logger.error(str(e))
