...
async def run(self):
    ...
    try:
        # =================user define code==================
        while True:
            location_name, _ = await self.get_put_location_by_rule([PickUp], pallet_type)
            if not location_name:
                ...
                await set_order_status_and_logger(self=self, logger_info=f'has find location:{location_name}')
                break
            await self.ts_delay(0.5)
        task_id, agv_id = await goto_location_reserve(self=self, location_name=location_name, agv_type=agv_type)
        task_id = await goto_location_load(self=self, location_name=location_name, agv_type=agv_type, agv_id=agv_id,
                                           pre_task_id=task_id, manage_mode=True, follow_task=True)
        # 搜索库区内库位
        while True:
            put_location, _ = await self.get_put_location_by_rule([DropOff], pallet_type)
            if put_location:
