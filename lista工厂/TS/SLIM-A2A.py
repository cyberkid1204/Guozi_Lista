...
desc = '项目描述'
para_template = {'SLIM_A2A': {'PickUp': 'str', 'pallet_type': 'str', 'DropOff': 'str'}}
operator_list = []
# ===================================基本配置区==========================================
...
async def run(self):
    ...
    try:
        # =================user define code==================
        while True:
            # self.get_fetch_locations_by_rule:在location集合中找到符合条件的取托盘位置，并锁定该位置，返回位置id和锁定结果，没有找到返回（None，None）
            load_location_name, _ = await self.get_fetch_locations_by_rule([PickUp], pallet_type)
            if not load_location_name:
                await set_order_status_and_logger(self=self,
                                                  logger_info=f'searching pallet with type of {pallet_type} to load in area:{PickUp}...')
            else:
                await set_order_status_and_logger(self=self, logger_info=f'has find location:{load_location_name}')
                break
            await self.ts_delay(0.5)
        # 搜索库区内库位
        while True:
            unload_put_location, _ = await self.get_put_location_by_rule([DropOff], pallet_type)
            if unload_put_location:
                await set_order_status_and_logger(self=self, logger_info=f'has find location:{unload_put_location}')
                break
            else:
                await set_order_status_and_logger(self=self,
        ...
        task_id, agv_id = await goto_location_reserve(self=self, location_name=load_location_name, agv_type=agv_type)

        task_id = await goto_location_load(self=self, location_name=load_location_name, agv_type=agv_type, agv_id=agv_id,
                                           pre_task_id=task_id, manage_mode=True, follow_task=True)
        task_id = await goto_location_unload(self=self, location_name=unload_put_location, agv_type=agv_type, agv_id=agv_id,
