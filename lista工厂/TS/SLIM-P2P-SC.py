...
desc = '项目描述'
para_template = {'SLIM_P2P': {'PickUp': 'str', 'DropOff': 'str'}}
operator_list = []
# ===================================基本配置区==========================================
...
async def run(self):
    ...
    try:
        # =================user define code==================
        check_Pickup = await check_location(self=self, location_name=PickUp)
        if not check_Pickup:
            raise Exception(f'location {PickUp} does not exist')
        check_Dropoff = await check_location(self=self, location_name=DropOff)
        if not check_Dropoff:
            raise Exception(f'location {DropOff} does not exist')
        # 校验库位是否有托盘
        while True:
            src_location_pallet = await self.get_location_pallet(PickUp)
            if not src_location_pallet:
                await set_order_status_and_logger(self=self, logger_info=f'There has no pallet on load location')
            else:
                await set_order_status_and_logger(self=self, logger_info=f'active')
                break
            await self.ts_delay(0.5)
        while True:
            dst_location_pallet = await self.get_location_pallet(DropOff)
            if dst_location_pallet:
                await set_order_status_and_logger(self=self, logger_info=f'There has pallet on unload location')
            else:
                break
            await self.ts_delay(0.5)
        # 尝试申请能去location的AGV，返回(task_id,agv_id)
        task_id, agv_id = await goto_location_reserve(self=self, location_name=PickUp, agv_type=agv_type)
        task_id = await goto_location_load(self=self, location_name=PickUp, agv_type=agv_type, agv_id=agv_id,
                                           pre_task_id=task_id, manage_mode=True, follow_task=True)
        task_id = await goto_location_unload(self=self, location_name=DropOff, agv_type=agv_type, agv_id=agv_id,
...
async def get_all_agv_type(self):
    """
    :param self:
    :return:
    """
    sql = "select string_agg(id||'' , ',') as agv_type from layer1_agv.agv_type;"
    result = await self.run_sql(sql)
    if not result:
        raise Exception('表layer1_agv.agv_type为空,没有agv可用于调度,订单无法执行.')
    agv_type = result[0].get('agv_type')
    await set_order_status_and_logger(self=self, logger_info=f'agv_type={agv_type}')
    return agv_type
