...
async def get_unload_location(self, area_name, pallet_type):
    while True:
        """
        get_put_location_by_rule(loc_ids,pallet_type,rule_filters,book):
        在location集合中找到符合条件的放托盘位置，并锁定该位置，返回位置id和锁定结果
        loc_ids : 目标位置列表，可以是字符串（location name）/int（location id）
        pallet_type:当前托盘类型
        rule_filters:托盘入库时的排序规则,默认按location ID正序排列
        book:False不锁定巷道，True或者不填都会锁定巷道
        返回值：(location_name, pallet_name);没有合适的位置时为（None，None）
        """
