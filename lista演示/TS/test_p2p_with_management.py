...
# @Author  : LuBowen
# @Number  : 20210509
# @FileName  :test_p2p_with_management.py
# @Time      :2022/5/17 9:24
...
desc = '项目描述'
para_template = {'test_p2p_with_management': {'src': 'str', 'dst': 'str', 'debug_mode': 'str'}}
operator_list = []
# ===================================基本配置区==========================================
# 作业车类型列表
...
async def run(self):
    ...
    error_reason = None
    agv_id = None
    debug_mode = True if self.debug_mode == 'on' else False
    try:
        report_to_mes_data = dict(
            status=order_status,
            taskNo=None,
            carNo=None,
            description='订单开始'
        ...
        # =================user define code==================
        task_id, agv_id = await self.goto_location_reserve(self.src, True, agv_type, None, None)
        task_id = await self.goto_location_load(self.src, True, agv_type, agv_id, task_id)
        task_id = await self.goto_location_unload(self.dst, False, agv_type, agv_id, task_id)
        # =================user define code==================
        order_status = 'finish'
        report_to_mes_data['description'] = '订单结束'
        await report_to_mes(self=self, url=report_finish_url, json_data=json.dumps(report_to_mes_data), blocking=True,
                            debug=debug_mode)
        return 0
        ...
        # cancel逻辑处理必须在此代码之后
        order_status = 'cancel'
        report_to_mes_data['status'] = order_status
        report_to_mes_data['description'] = '订单取消'
        await report_to_mes(self=self, url=report_cancel_url, json_data=json.dumps(report_to_mes_data), blocking=True,
                            debug=debug_mode)
        return 1
        ...
        order_status = 'error'
        report_to_mes_data['status'] = order_status
        report_to_mes_data['description'] = f'订单报错,报错原因:{error_reason}'
        await report_to_mes(self=self, url=report_error_url, json_data=json.dumps(report_to_mes_data), blocking=True,
                            debug=debug_mode)
        return 504
    finally:
        # 订单结束释放所有已上锁的库位
        ...
        agv_id_info = f'而后请重置执行本订单的第{agv_id}号agv!' if agv_id else '!'
        if order_status == 'cancel':
            await set_order_status_and_logger(self=self,
                                              logger_info=f'锁定的库位已全部释放,请手动处理掉处于handle状态的任务, {agv_id_info}')
        elif order_status == 'error':
            while True:
                await set_order_status_and_logger(self=self, logger_info=f'订单报错,报错原因:{error_reason}!')
