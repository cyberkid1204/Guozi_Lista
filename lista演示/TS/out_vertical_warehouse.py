...
# @Author  : LuBowen
# @Number  : 20210509
# @FileName  :out_vertical_warehouse_test.py
# @Time      :2022/2/14 10:08
...
desc = '垂直货柜出库'
# task={'task':[{dst:location1, pallet_type:5},...]}
para_template = {'out_vertical_warehouse': {'tasks': 'str'}}
operator_list = []
picking_agv_type = [1]
...
async def run(self):
    try:
        ...
        dst = task_list[0].get('dst')
        task_id, agv_id = await self.goto_location_reserve(dst, True, picking_agv_type, None, None)
...
async def picking_load(self, task_list, agv_id, task_id=None):
    ...
    for task in task_list:
        ...
        location_name, pallet_name = await self.get_fetch_locations_by_rule(['area_vertical'], int(pallet_type))
        ...
        if not current_tray:
            ...
            while True:
                return_tray_ready = await return_tray(self=self, machine_id=machineID, tray_no=current_tray, position=current_position)
        ...
        if current_tray != tray:
            while True:
                ...
                if check_status:
                    ...
                    break
        location_pallet = await self.get_location_pallet(location_name)
        task_id = await self.goto_location_load(location_name, True, picking_agv_type,
                                                agv_id, task_id, layer, None, False)
        await self.del_pallet(location_pallet)
...
async def picking_unload(self, task_list, agv_id, task_id=None):
    ...
    for task in task_list:
        ...
        if not layer:
            continue
        dst = task.get('dst')
        task_id = await self.goto_location_unload(dst, True, picking_agv_type,
                                                  agv_id, task_id, layer, None, False)
    return task_list, task_id
